...
export default function Ratings() {
    ...
    queryFn: async () => {
      if (isProvider) {
        const { data, error } = await supabase
          .from("contracts")
          .select(
            "*, projects(title, status), profiles:association_id(full_name, avatar_url)",
          )
        ...
        const { data, error } = await supabase
          .from("contracts")
          .select(
            "*, projects(title, status), profiles:provider_id(full_name, avatar_url)",
          )
  ...
  const ratedContractIds = new Set(
    existingRatings?.map((r) => r.contract_id) ?? [],
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isProvider
                ? "قيّم الجمعيات بعد إتمام العقود"
                : "قيّم مقدمي الخدمات بعد إتمام العقود"}
            </p>
          </div>
        </div>
              ...
              {[
                {
                  ...
                  value: quality,
                  onChange: setQuality,
                  desc: "مستوى جودة المخرجات والنتائج",
                },
                {
                  label: "الالتزام بالوقت",
                  value: timing,
                  onChange: setTiming,
                  desc: "الالتزام بالمواعيد المحددة",
                },
                {
                  label: "التواصل",
                  value: communication,
                  onChange: setCommunication,
                  desc: "مستوى التواصل والاستجابة",
                },
        ...
        {unrated.length > 0 && (
          <div className="space-y-3">
            <h2 className="text-lg font-semibold">
              بانتظار التقييم ({unrated.length})
            </h2>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">
                          {(contract as any).projects?.title || "طلب"}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          {profile?.full_name ||
                            (isProvider ? "جمعية" : "مقدم خدمة")}
                        </p>
                      </div>
                    </div>
                    <Button
                      size="sm"
                      onClick={() => openRating(contract.id)}
                      disabled={activeContract === contract.id}
                    >
                      تقييم
                    </Button>
                  </CardContent>
