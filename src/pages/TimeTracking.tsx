...
import {
  ...
  useAssignedProjects,
  useCreateTimeLog,
} from "@/hooks/useProviderTimeLogs";
import {
  TimeEntryForm,
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              سجّل ساعات عملك على الطلبات المسندة إليك
            </p>
          </div>
        </div>
        ...
        {projects && projects.length > 0 && (
          <WorkTimer onStop={handleTimerStop} />
        )}
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <EmptyState
                icon={ClipboardList}
                title="لا توجد ساعات مسجلة"
                description="سجل ساعات عملك على الطلبات المسندة إليك"
              />
            ) : (
              <div className="space-y-2">
                {timeLogs.map((log: any) => {
                  const st =
                    approvalLabels[log.approval] ?? approvalLabels.pending;
                  return (
                    <div
                      key={log.id}
                      className={`flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-muted/30 transition-colors ${st.border}`}
                    >
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <p className="text-xs text-destructive mt-1 flex items-center gap-1 cursor-help">
                                    <AlertCircle className="h-3 w-3" /> سبب
                                    الرفض: {log.rejection_reason}
                                  </p>
                                </TooltipTrigger>
                                <TooltipContent>
                      ...
                      <Badge variant={st.variant} className="shrink-0 ms-3">
                        {st.label}
                      </Badge>
                    </div>
                  );
