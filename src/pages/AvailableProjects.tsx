  ...
  const { data: projects, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("projects")
        .select(
          "*, categories(*), regions(*), profiles!projects_association_id_fkey(full_name, avatar_url, organization_name)",
        )
        .eq("status", "open")
        .eq("is_private", false)
      ...
      if (sortBy === "newest")
        ...
        query = query.order("budget", { ascending: false, nullsFirst: true });
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                تصفح الطلبات المفتوحة وقدّم عروضك
              </p>
            </div>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <Input
                  ...
                  placeholder="من"
                  className="w-[80px] h-9 bg-background"
                  min={0}
                />
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="لم يتم العثور على طلبات مطابقة. جرّب تغيير الفلاتر أو تحقق لاحقاً."
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map((p) => (
              <ProviderProjectCard
                key={p.id}
                project={p}
                hasBid={myBidProjectIds?.has(p.id)}
                onViewDetails={(id) =>
                  guardAction(() => navigate(`/available-projects/${id}`))
                }
              />
            ))}
          </div>
