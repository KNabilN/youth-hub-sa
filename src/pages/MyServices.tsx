...
export default function MyServices() {
  ...
  const handleCreateDraft = (values: ServiceFormValues) => {
    ...
    createService.mutate(
      {
        ...values,
        title: values.title || "خدمة جديدة (مسودة)",
        approval: "draft",
  ...
  const handleEdit = (values: ServiceFormValues) => {
    ...
    updateService.mutate(
      { id: editingId, ...values } as Parameters<
        typeof updateService.mutate
      >[0],
      {
        onSuccess: () => {
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                أدر خدماتك المقدمة وأضف خدمات جديدة
              </p>
            </div>
          </div>
          <Button
            onClick={() => guardPublish(() => setFormOpen(true))}
            className="bg-gradient-to-l from-primary to-primary/90 hover:from-primary/90 hover:to-primary shadow-md"
            disabled={!canPublish}
          >
          ...
          <Alert variant="default" className="border-warning bg-warning/10">
            ...
            <AlertDescription className="flex flex-wrap items-center gap-3">
              <span className="text-sm">
                {blockReason === "verification" &&
                  "حسابك غير موثق. يجب توثيق الحساب قبل نشر الخدمات."}
                {blockReason === "profile" &&
                  "ملفك الشخصي غير مكتمل. أكمل البيانات المطلوبة قبل نشر الخدمات."}
                {blockReason === "portfolio" &&
        ...
        {isLoading ? (
          ...
          <EmptyState
            icon={Layers}
            title="لا توجد خدمات"
            description="أضف خدمتك الأولى لتبدأ في تلقي الطلبات"
            actionLabel="إضافة خدمة"
            onAction={() => guardPublish(() => setFormOpen(true))}
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {services.map((s) => (
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <Alert variant="default" className="border-warning bg-warning/10">
              ...
              <AlertDescription className="text-xs">
                تعديل الخدمة سيعيدها إلى حالة "قيد المراجعة" حتى يوافق عليها
                المدير مجدداً.
              </AlertDescription>
            </Alert>
            {editingService && (
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                سيتم نقل هذه الخدمة إلى سلة المحذوفات لمدة 30 يوماً، ويمكنك
                استرجاعها خلال هذه المدة.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
