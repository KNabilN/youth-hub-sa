...
const statusBorders: Record<string, string> = {
  pending: "border-e-4 border-warning/30",
  approved: "border-e-4 border-success/30",
  rejected: "border-e-4 border-destructive/30",
  processed: "border-e-4 border-info/30",
};
...
export default function Earnings() {
  ...
  const handleWithdraw = () => {
    ...
    createWithdrawal.mutate(
      { amount: confirmData.amount, escrow_id: confirmData.escrowId },
      {
        onSuccess: () => {
      ...
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="ستظهر أرباحك هنا بعد إتمام مشاريعك بنجاح"
          />
        ) : (
          <EarningsSummary
            ...
            onWithdraw={(escrowId, amount, projectTitle) =>
              setConfirmData({ escrowId, amount, projectTitle })
            }
          />
        )}
            ...
            <CardContent>
              <div className="space-y-2">
                {withdrawals?.map((w: any) => (
                  <div
                    key={w.id}
                    className={`flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-muted/30 transition-colors ${statusBorders[w.status] ?? ""}`}
                  >
                    <div>
                      {w.withdrawal_number && (
            ...
            <div className="space-y-3 py-4">
              ...
              <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 space-y-1">
                ...
                <p className="text-2xl font-bold text-primary">
                  {confirmData?.amount.toLocaleString()} ر.س
                </p>
              </div>
            </div>
            <DialogFooter>
