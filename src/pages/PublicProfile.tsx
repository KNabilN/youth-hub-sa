...
export default function PublicProfile() {
  ...
  const { profile, role, services, portfolio, ratings, savesCount, projects } =
    usePublicProfile(id);
  ...
  const showSupportButton =
    currentUserRole === "donor" && role.data === "youth_association";
  ...
  const avgRating = r.length
    ? r.reduce(
        (s: number, rt: any) =>
          s + (rt.quality_score + rt.timing_score + rt.communication_score) / 3,
        0,
      ) / r.length
    : 0;

  const skills: string[] = (p as any)?.skills ?? [];
  const qualifications: { title: string; description?: string }[] =
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {role.data && (
                <Badge variant="secondary">
                  {roleLabels[role.data] ?? role.data}
                </Badge>
              )}
            </div>
      ...
      <div className="px-4 sm:px-8 mt-8">
        <Tabs defaultValue="about" dir="rtl">
          <TabsList className="w-full justify-start overflow-x-auto flex-nowrap scrollbar-hide bg-muted/50 h-auto p-1 gap-1 rounded-xl">
            <TabsTrigger value="about" className="gap-1.5 rounded-lg">
              <UserIcon className="h-4 w-4" /> نبذة
            </TabsTrigger>
            {!isAssociation && skills.length > 0 && (
              <TabsTrigger value="skills" className="gap-1.5 rounded-lg">
            ...
            {isAssociation ? (
              ...
              <TabsTrigger value="services" className="gap-1.5 rounded-lg">
                <Briefcase className="h-4 w-4" /> الخدمات
              </TabsTrigger>
            )}
            {!isAssociation && (
              <TabsTrigger value="portfolio" className="gap-1.5 rounded-lg">
            ...
            {!isAssociation && qualifications.length > 0 && (
              ...
              >
                <GraduationCap className="h-4 w-4" /> المؤهلات
              </TabsTrigger>
            )}
            <TabsTrigger value="ratings" className="gap-1.5 rounded-lg">
              <MessageSquare className="h-4 w-4" /> التقييمات
            </TabsTrigger>
          </TabsList>
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center gap-3 mb-2">
                            <Avatar className="h-9 w-9">
                              <AvatarImage
                                src={rt.profiles?.avatar_url || undefined}
                              />
                              <AvatarFallback>
                                {rt.profiles?.full_name?.[0] ?? "؟"}
                              </AvatarFallback>
                            </Avatar>
                            <div className="flex-1">
                              <p className="text-sm font-medium">
                                {rt.profiles?.full_name ?? "مستخدم"}
                              </p>
                              <p className="text-xs text-muted-foreground">
                                {format(
                                  new Date(rt.created_at),
                                  "dd MMMM yyyy",
                                  { locale: ar },
                                )}
                              </p>
