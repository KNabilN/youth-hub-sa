...
function getDisplayName(
  profile: { full_name: string; organization_name: string | null } | null,
): string {
  if (!profile) return "—";
...
export default function AdminContracts() {
  ...
  const filtered = (contracts ?? []).filter((c) => {
    if (statusFilter !== "all" && c.signatureStatus !== statusFilter)
      return false;
    if (search) {
  ...
  const statusCounts = (contracts ?? []).reduce(
    (acc, c) => {
      acc[c.signatureStatus] = (acc[c.signatureStatus] || 0) + 1;
      return acc;
    },
      ...
      <div className="space-y-6">
              ...
              <h1 className="text-2xl font-bold flex items-center gap-2">
                ...
                {(statusCounts.partial ?? 0) > 0 && (
                  <Badge className="bg-warning/15 text-warning border-warning/30">
                    {statusCounts.partial} جزئي التوقيع
                  </Badge>
                )}
              </h1>
              <p className="text-sm text-muted-foreground">
                متابعة حالة توقيع العقود بين الجمعيات ومقدمي الخدمات
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="space-y-1.5">
            ...
            <Input
              placeholder="بحث بالعنوان أو رقم الطلب أو اسم الطرف..."
              value={search}
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                <SelectItem value="all">
                  الكل ({contracts?.length ?? 0})
                </SelectItem>
                {(
                  Object.entries(signatureLabels) as [SignatureStatus, string][]
                ).map(([k, v]) => (
                  ...
                  <TableBody>
                    {paged.map((c) => (
                      <TableRow
                        ...
                        onClick={() =>
                          navigate(`/admin/projects/${(c as any).project_id}`)
                        }
                      >
                        <TableCell className="font-mono text-sm font-semibold">
                          {c.project?.request_number || "—"}
                        </TableCell>
                        <TableCell
                        ...
                        <TableCell
                          ...
                          title={getDisplayName(c.association)}
                        >
                          {getDisplayName(c.association)}
                        </TableCell>
                        <TableCell
                          className="max-w-[120px] truncate"
                          title={getDisplayName(c.provider)}
                        >
                          {getDisplayName(c.provider)}
                        </TableCell>
                        <TableCell>
                          <Badge className={signatureColors[c.signatureStatus]}>
                            {signatureLabels[c.signatureStatus]}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-sm">
                          {c.association_signed_at ? (
                            <span className="text-primary">
                              {format(
                                new Date(c.association_signed_at),
                                "yyyy/MM/dd",
                                { locale: ar },
                              )}
                            </span>
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                navigate(
                                  `/admin/projects/${(c as any).project_id}`,
                                )
                              }
                            >
        ...
        filters={[
          {
            key: "status",
            label: "حالة التوقيع",
            options: Object.entries(signatureLabels).map(([k, v]) => ({
              value: k,
              label: v,
            })),
          },
 ...
 `,
          ...
          if (filters.status !== "all")
            rows = rows.filter(
              (c: any) => c.signatureStatus === filters.status,
            );
          const colMap: Record<string, (c: any) => string> = {
            request_number: (c) => c.project?.request_number || "",
            project_title: (c) => c.project?.title || "",
            association: (c) =>
              c.association?.organization_name ||
              c.association?.full_name ||
              "",
            provider: (c) => c.provider?.full_name || "",
            status: (c) =>
              signatureLabels[c.signatureStatus as SignatureStatus] || "",
            association_signed: (c) =>
              c.association_signed_at?.slice(0, 10) || "لم يوقّع",
            provider_signed: (c) =>
              c.provider_signed_at?.slice(0, 10) || "لم يوقّع",
            created_at: (c) => c.created_at?.slice(0, 10) || "",
          };
          const activeCols = exportColumns.filter((col) =>
            cols.includes(col.key),
          );
          return {
            headers: activeCols.map((col) => col.label),
            rows: rows.map((c: any) =>
              activeCols.map((col) => colMap[col.key]?.(c) ?? ""),
            ),
          };
        }}
      />
