...
export default function AdminDisputes() {
  ...
  const filtered = (disputes ?? []).filter((d: any) => {
    if (search) {
      ...
      if (!title.includes(q) && !name.includes(q) && !dn.includes(q))
        return false;
    }
  ...
  return (
    <DashboardLayout>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                متابعة وحل النزاعات بين الأطراف
              </p>
            </div>
          </div>
        </div>
                      ...
                      <TableRow key={d.id} id={`row-${d.id}`}>
                        ...
                        <TableCell>
                          <Select
                            value={d.status}
                            onValueChange={(v) =>
                              handleStatusChange(d, v as DisputeStatus)
                            }
                          >
      ...
      <ExportDialog
        ...
        filters={[
          {
            ...
            options: allDisputeStatuses.map((s) => ({
              value: s,
        ...
        onExport={async (cols, filters) => {
          const { data } = await supabase
            .from("disputes")
            .select(
              "dispute_number, description, status, created_at, projects(title), profiles!disputes_raised_by_fkey(full_name)",
            );
          let rows = data ?? [];
          if (filters.status !== "all")
            rows = rows.filter((d: any) => d.status === filters.status);
          ...
          const activeCols = disputeExportColumns.filter((c) =>
            cols.includes(c.key),
          );
          return {
            headers: activeCols.map((c) => c.label),
            rows: rows.map((d: any) =>
              activeCols.map((c) => colMap[c.key]?.(d) ?? ""),
            ),
          };
        }}
      />
      <ConfirmDialog
        ...
        onConfirm={() => {
          softDelete.mutate(
            { table: "disputes", id: deleteTarget.id },
            {
              onSuccess: () => {
