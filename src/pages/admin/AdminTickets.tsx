...
export default function AdminTickets() {
  ...
  const pagination = usePagination("admin-tickets");
  ...
  return (
    <DashboardLayout>
                      ...
                      <TableRow key={t.id} id={`row-${t.id}`}>
                        ...
                        <TableCell>
                          <Select
                            value={t.status}
                            onValueChange={(v) =>
                              handleStatusChange(t.id, v as TicketStatus)
                            }
                          >
      ...
      <ExportDialog
        ...
        onExport={async (cols, filters) => {
          const { data } = await supabase
            ...
            .select(
              "ticket_number, subject, priority, status, created_at, profiles!support_tickets_user_id_fkey(full_name, organization_name)",
            );
          let rows = data ?? [];
          if (filters.status !== "all")
            rows = rows.filter((t: any) => t.status === filters.status);
          if (filters.priority !== "all")
            rows = rows.filter((t: any) => t.priority === filters.priority);
          const colMap: Record<string, (t: any) => string> = {
            ticket_number: (t) => t.ticket_number || "",
            subject: (t) => t.subject || "",
            user: (t) =>
              (t.profiles as any)?.organization_name ||
          ...
          const activeCols = ticketExportColumns.filter((c) =>
            cols.includes(c.key),
          );
          return {
            headers: activeCols.map((c) => c.label),
            rows: rows.map((t: any) =>
              activeCols.map((c) => colMap[c.key]?.(t) ?? ""),
            ),
          };
        }}
      />
      <ConfirmDialog
        ...
        onConfirm={() => {
          softDelete.mutate(
            { table: "support_tickets", id: deleteTarget.id },
            {
              onSuccess: () => {
