...
export default function AdminUserDetail() {
  ...
  const handleToggleVerify = () => {
    toggleVerify.mutate(
      ...
      {
        onSuccess: () =>
          toast.success(user.is_verified ? "تم إلغاء التوثيق" : "تم التوثيق"),
  ...
  const handleSuspendConfirm = () => {
    if (!suspensionReason.trim()) {
      toast.error(
        user.is_suspended
          ? "يرجى إدخال سبب إلغاء التعليق"
          : "يرجى إدخال سبب التعليق",
      );
      return;
    }
    toggleSuspend.mutate(
      {
        ...
        suspension_reason: user.is_suspended ? "" : suspensionReason,
      },
      {
        onSuccess: async () => {
          await logAudit(
          ...
          toast.success(
            user.is_suspended ? "تم إلغاء التعليق" : "تم تعليق الحساب",
          );
    ...
    <DashboardLayout>
      ...
      <div className="max-w-6xl mx-auto px-4 pb-12 space-y-6">
        {pendingEdit && (
          <Alert className="border-warning/40 bg-warning/10">
            <AlertDescription className="flex items-center justify-between gap-3">
              <span>
                هذا المستخدم لديه طلب تعديل على ملفه الشخصي بانتظار المراجعة.
              </span>
        ...
        {user.is_suspended && user.suspension_reason && (
          <Alert variant="destructive">
            <Ban className="h-4 w-4" />
            <AlertDescription>
              <span className="font-semibold">سبب التعليق:</span>{" "}
        ...
        <Tabs defaultValue="profile" className="w-full" dir="rtl">
          <TabsList className="w-full flex-wrap h-auto gap-1 p-1">
            ...
            {role === "donor" && (
              <TabsTrigger value="donations">المنح</TabsTrigger>
            )}
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <InfoField
                      icon={UserCircle}
                      label="الاسم"
                      value={user.contact_officer_name}
                    />
                    <InfoField
                      icon={Phone}
                      label="الهاتف"
                      value={user.contact_officer_phone}
                    />
                    <InfoField
                      icon={Mail}
                      label="البريد الإلكتروني"
                      value={user.contact_officer_email}
                    />
                    <InfoField
                      icon={Briefcase}
                      label="الصفة"
                      value={user.contact_officer_title}
                    />
                  </div>
                </CardContent>
              </Card>
                    ...
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge variant="outline">
                          {approvalLabels[s.approval] ?? s.approval}
                        </Badge>
                      </div>
                  ...
                  <Card key={c.id}>
                    <CardContent className="p-4">
                      ...
                      <div className="flex gap-3 text-sm text-muted-foreground mt-1">
                        <span>
                          مقدم الخدمة:{" "}
                          {c.provider_signed_at ? "وقّع" : "لم يوقّع"}
                        </span>
                        <span>
                          • الجمعية:{" "}
                          {c.association_signed_at ? "وقّعت" : "لم توقّع"}
                        </span>
                      </div>
                  ...
                  <Card key={t.id}>
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge variant="outline">
                          {approvalLabels[t.approval] ?? t.approval}
                        </Badge>
                      </div>
                      <div className="flex gap-3 text-sm text-muted-foreground mt-1">
                  ...
                  <Card key={d.id}>
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                        <span className="font-medium">
                          {d.projects?.title ??
                            d.micro_services?.title ??
                            d.profiles?.organization_name ??
                            d.profiles?.full_name ??
                            "منحة عامة"}
                        </span>
                        <Badge variant="outline">
                          {donationStatusLabels[d.donation_status] ??
                            d.donation_status}
                        </Badge>
                      </div>
                      <div className="flex gap-3 text-sm text-muted-foreground mt-2">
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {user.is_suspended ? "إلغاء تعليق الحساب" : "تعليق الحساب"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div>
              <Label>
                {user.is_suspended ? "سبب إلغاء التعليق *" : "سبب التعليق *"}
              </Label>
              <Textarea
