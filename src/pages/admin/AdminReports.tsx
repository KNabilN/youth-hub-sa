      ...
      {payload.map((entry: any, i: number) => (
        <div key={i} className="flex items-center gap-2 text-xs">
          ...
          <span className="text-muted-foreground">
            {(entry.name === "value" ? "القيمة" : entry.name) ?? entry.dataKey}:
          </span>
...
const chartCardCls =
  "rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-300 border border-border/50 overflow-hidden";
...
export default function AdminReports() {
  ...
  const { data: servicesByCategory } = useQuery({
    queryKey: [
      "admin-report-services-category",
      dateFrom,
      dateTo,
      regionId,
      cityId,
  ...
  const { data: serviceApprovalStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const counts: Record<string, number> = {};
      (data ?? []).forEach((s: any) => {
        counts[s.approval] = (counts[s.approval] || 0) + 1;
      });
  ...
  const { data: monthlyEscrow } = useQuery({
    ...
    queryFn: async () => {
      ...
      return Object.entries(months)
        .sort()
        .map(([month, v]) => ({ month, total: v.total, released: v.released }));
    },
  });

  const { data: donorAnalytics } = useQuery({
    queryKey: [
      "admin-report-donor-analytics",
      dateFrom,
      dateTo,
      regionId,
      cityId,
    ...
    queryFn: async () => {
      let q = supabase
        .from("donor_contributions")
        .select(
          "donor_id, amount, project_id, association_id, projects(title), profiles!donor_contributions_donor_id_fkey(full_name, organization_name)",
        )
        .gte("created_at", dateFrom)
        .lte("created_at", dateTo);
      if ((regionId || cityId) && regionProjectIds) {
        if (regionProjectIds.length === 0)
          return {
            totalDonors: 0,
            totalGrants: 0,
            byProject: [],
      ...
      const donorMap: Record<
        string,
        { name: string; associations: Set<string>; total: number }
      > = {};
      (data ?? []).forEach((d: any) => {
        ...
        if (d.association_id)
          donorMap[d.donor_id].associations.add(d.association_id);
      ...
      return {
        ...
        totalGrants,
        byProject: Object.entries(byProject).map(([name, amount]) => ({
          name,
          amount,
        })),
        perDonor,
  ...
  const { data: insights } = useQuery({
    ...
    queryFn: async () => {
      ...
      const heldEscrow = (escrowData ?? []).reduce(
      ...
      const activeDonors = new Set(
        (donorData ?? []).map((d: any) => d.donor_id),
      ).size;
  ...
  const exportUsers = async () => {
    const { data } = await supabase
      .from("profiles")
      .select("full_name, phone, organization_name, is_verified, created_at")
      .gte("created_at", dateFrom)
      .lte("created_at", dateTo);
    downloadXLSX(
      "users.xlsx",
      ["الاسم", "الهاتف", "المنظمة", "موثق", "تاريخ الانضمام"],
      (data ?? []).map((u: any) => [
        u.full_name,
        u.phone ?? "",
        u.organization_name ?? "",
  ...
  const exportProjects = async () => {
    let q = supabase
      .from("projects")
      .select(
        "id, title, status, budget, created_at, regions(name), categories(name)",
      )
      .gte("created_at", dateFrom)
      .lte("created_at", dateTo);
    if (regionId) q = q.eq("region_id", regionId);
  ...
  const exportServices = async () => {
    let q = supabase
      .from("micro_services")
      .select(
        "title, price, approval, created_at, categories(name), profiles!micro_services_provider_id_fkey(full_name)",
      )
      .is("deleted_at", null)
      .gte("created_at", dateFrom)
      .lte("created_at", dateTo);
    ...
    downloadXLSX(
      "services.xlsx",
      ["العنوان", "مقدم الخدمة", "السعر", "التصنيف", "الحالة", "تاريخ الإنشاء"],
      (data ?? []).map((s: any) => [
        s.title,
  ...
  const exportInvoices = async () => {
    ...
    if ((regionId || cityId) && regionProjectIds?.length) {
      const { data: escrows } = await supabase
        .from("escrow_transactions")
        .select("id")
        .in("project_id", regionProjectIds);
      escrowIds = (escrows ?? []).map((e: any) => e.id);
    }
    let q = supabase
      .from("invoices")
      .select("invoice_number, amount, commission_amount, created_at")
      .gte("created_at", dateFrom)
      .lte("created_at", dateTo);
    if (escrowIds !== null) {
      if (escrowIds.length === 0) {
        downloadXLSX(
          "invoices.xlsx",
          ["رقم الفاتورة", "المبلغ", "العمولة", "تاريخ الإنشاء"],
          [],
        );
        return;
      }
  ...
  const chartDonorByProject = topNWithOthersAmount(
    donorAnalytics?.byProject ?? [],
  );
  ...
  const insightCards = [
    {
      ...
      value: insights?.servicesCount ?? 0,
      icon: Package,
      color: "text-info bg-info/10",
    },
    {
      label: "المشاريع المكتملة",
      value: insights?.completedProjects ?? 0,
      icon: CheckCircle2,
      color: "text-success bg-success/10",
    },
    {
      label: "العقود",
      value: insights?.contractsCount ?? 0,
      icon: FileSignature,
      color: "text-violet-600 bg-violet-100",
    },
    {
      label: "الضمانات المحتجزة",
      value: `${(insights?.heldEscrow ?? 0).toLocaleString()} ر.س`,
      icon: Shield,
      color: "text-warning bg-warning/10",
    },
    {
      label: "المانحين النشطين",
      value: insights?.activeDonors ?? 0,
      icon: HeartHandshake,
      color: "text-rose-600 bg-rose-100",
    },
    {
      label: "تذاكر الدعم",
      value: insights?.ticketsCount ?? 0,
      icon: LifeBuoy,
    ...
    <DashboardLayout>
      <div className="space-y-6">
          ...
          <div className="flex gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={exportFinancial}>
                  تصدير المالية
                </DropdownMenuItem>
        ...
        <div ref={reportContentRef} className="space-y-6">
          ...
          <h1
            className="text-2xl font-bold text-center hidden"
            style={{ display: "none" }}
          >
            تقرير تحليلات المنصة
          </h1>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              <CardContent className="pt-6">
                <p className="text-sm text-muted-foreground">طلبات الجمعيات</p>
            ...
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="text-2xl font-bold">
                  {(stats?.revenue ?? 0).toLocaleString()} ر.س
                </p>
              </CardContent>
            </Card>
          </div>
                  ...
                  <BarChart data={usersByRole ?? []} margin={{ top: 20 }}>
                    ...
                    >
                      {(usersByRole ?? []).map((_: any, i: number) => (
                        <Cell
                          key={i}
                          fill={`url(#roleGrad${i % ROLE_COLORS.length})`}
                        />
                      ))}
                      <LabelList
                        dataKey="value"
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    <Pie
                      ...
                      animationEasing="ease-out"
                    >
                      {chartServicesByCategory.map((_: any, i: number) => (
                        <Cell
                          key={i}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card className={chartCardCls}>
              <CardHeader>
                <CardTitle className="text-lg text-center">المنح</CardTitle>
              </CardHeader>
              <CardContent className="p-6">
                <ResponsiveContainer width="100%" height={260}>
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    <Pie
                      ...
                      animationEasing="ease-out"
                    >
                      {(serviceApprovalStats ?? []).map((_: any, i: number) => (
                        <Cell
                          key={i}
                          fill={STATUS_COLORS[i % STATUS_COLORS.length]}
                        />
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-muted/50 rounded-lg p-4 text-center">
                  ...
                  <p className="text-2xl font-bold">
                    {donorAnalytics?.totalDonors ?? 0}
                  </p>
                </div>
                <div className="bg-muted/50 rounded-lg p-4 text-center">
                  <p className="text-sm text-muted-foreground">إجمالي المنح</p>
                  <p className="text-2xl font-bold">
                    {(donorAnalytics?.totalGrants ?? 0).toLocaleString()} ر.س
                  </p>
                </div>
              </div>
                ...
                <ResponsiveContainer width="100%" height={320}>
                  ...
                  >
                    ...
                    <Bar
                      yAxisId="left"
                      dataKey="charities"
                      fill="url(#donorCharitiesGrad)"
                      name="عدد الجمعيات المدعومة"
                      radius={[6, 6, 0, 0]}
                      animationDuration={800}
                      animationEasing="ease-out"
                    >
                    ...
                    <Bar
                      yAxisId="right"
                      dataKey="amount"
                      fill="url(#donorAmountGrad)"
                      name="إجمالي المنح (ر.س)"
                      radius={[6, 6, 0, 0]}
                      animationDuration={800}
                      animationEasing="ease-out"
                    >
