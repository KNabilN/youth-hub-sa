...
const pageGroups = {
  landing: {
    ...
    description:
      "Hero، الإحصائيات، المميزات، الطلبات، الخدمات، لماذا المنصة، CTA، آراء العملاء، التواصل",
    icon: Globe,
    keys: [
      "hero",
      "stats",
  ...
  return (
    <div className="space-y-3">
      ...
      <Button
        ...
        size="sm"
        onClick={() =>
          onChange([...items, { title: "", desc: "", icon: "users" }])
        }
      >
        <Plus className="h-4 w-4 me-1" />
        إضافة ميزة
...
function TestimonialsEditor({
  ...
  items: { name: string; org: string; text: string; rating: number }[];
  ...
  return (
    <div className="space-y-3">
      ...
      <Button
        ...
        size="sm"
        onClick={() =>
          onChange([...items, { name: "", org: "", text: "", rating: 5 }])
        }
      >
        <Plus className="h-4 w-4 me-1" />
        إضافة شهادة
  ...
  const { data: services, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("micro_services")
        .select(
          "id, title, is_featured, approval, provider:profiles!micro_services_provider_id_fkey(full_name)",
        )
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {featuredCount} مميزة من {all.length}
        </span>
      </div>
      ...
      <div className="border rounded-lg max-h-72 overflow-y-auto divide-y">
        {filtered.map((s: any) => (
          <div
            key={s.id}
            className={`flex items-center justify-between px-3 py-2.5 hover:bg-muted/50 transition-colors ${s.is_featured ? "bg-primary/5" : ""}`}
          >
        ...
        {filtered.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-4">
            {q ? "لا توجد نتائج" : "لا توجد خدمات معتمدة"}
          </p>
        )}
      </div>
    </div>
  ...
  const { data: projects, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("projects")
        .select(
          "id, title, is_featured, status, association:profiles!projects_association_id_fkey(full_name, organization_name)",
        )
        .eq("status", "open")
        .eq("is_private", false)
        .is("deleted_at", null)
  ...
  const filtered = q
    ? all.filter(
        (p: any) =>
          ...
          p.association?.organization_name?.toLowerCase().includes(q) ||
          p.association?.full_name?.toLowerCase().includes(q),
      )
    : all;
    ...
    <div className="space-y-2">
      ...
      <div className="border rounded-lg max-h-72 overflow-y-auto divide-y">
        {filtered.map((p: any) => (
          <div
            key={p.id}
            className={`flex items-center justify-between px-3 py-2.5 hover:bg-muted/50 transition-colors ${p.is_featured ? "bg-primary/5" : ""}`}
          >
            <div className="min-w-0 flex-1 me-3">
              <div className="flex items-center gap-1.5">
                {p.is_featured && (
              ...
              <p className="text-xs text-muted-foreground truncate">
                {p.association?.organization_name || p.association?.full_name}
              </p>
            </div>
            <Switch
        ...
        {filtered.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-4">
            {q ? "لا توجد نتائج" : "لا توجد طلبات مفتوحة"}
          </p>
        )}
      </div>
    </div>
  ...
  const handleSave = () => {
    update.mutate(
      { sectionKey, content },
      {
        onSuccess: () =>
          toast.success(
            `تم حفظ قسم "${sectionLabels[sectionKey] || sectionKey}"`,
          ),
        ...
        return (
          <div className="space-y-3">
            ...
            <JsonFieldEditor
              ...
              value={content.login_text || ""}
              onChange={(v) => set("login_text", v)}
            />
            <JsonFieldEditor
              label="نص إنشاء حساب"
              value={content.register_text || ""}
              onChange={(v) => set("register_text", v)}
            />
          </div>
        );
      case "footer":
...
function PageSelectionGrid({ onSelect }: { onSelect: (key: PageKey) => void }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(
        Object.entries(pageGroups) as [PageKey, (typeof pageGroups)[PageKey]][]
      ).map(([key, group]) => {
...
export default function AdminCMS() {
  ...
  const filteredSections = selectedGroup
    ? (selectedGroup.keys as readonly string[])
        .map((key) => (sections ?? []).find((s: any) => s.section_key === key))
        .filter(Boolean)
    : [];
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {selectedGroup
                ? selectedGroup.label
                : "اختر الصفحة أو القالب الذي تريد تعديله"}
            </p>
          </div>
        </div>
        <Separator />
