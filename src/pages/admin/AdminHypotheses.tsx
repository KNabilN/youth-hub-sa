...
const AUTOMATED_HYPOTHESES = new Set([
  1, 2, 3, 5, 7, 8, 9, 10, 11, 12, 14, 15, 17, 18, 23, 24, 25,
]);
...
function AdminHypotheses() {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      const progressPct = Math.round(
        ((counts.verified + counts.not_verified) / all.length) * 100,
      );
  ...
  const progressPct =
    all.length > 0
      ? Math.round(((counts.verified + counts.not_verified) / all.length) * 100)
      : 0;
      ...
      <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                تتبع فرضيات نجاح النظام — المؤشرات تُحسب آلياً من بيانات النظام
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <KPICard
            ...
            label="التقدم الكلي"
            value={`${progressPct}%`}
            sub={`${counts.verified + counts.not_verified} من ${all.length}`}
            color="text-primary"
          />
          <KPICard
            icon={CheckCircle2}
  ...
  return (
    <Card className="group hover:shadow-md transition-shadow">
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                <Progress
                  value={progressValue}
                  className={`h-1.5 w-32 ${progressColor}`}
                />
                {isAutomated && (
                  <Badge
                    variant="outline"
      ...
      <CardContent className="pt-0 space-y-3">
        ...
        {isAutomated && (
          <HypothesisMetricsPanel
            hypothesisNumber={h.hypothesis_number}
            metrics={metrics}
          />
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="ملاحظات..."
              onBlur={(e) => {
                if (e.target.value !== h.admin_notes)
                  onUpdate(h.id, "admin_notes", e.target.value);
              }}
            />
          </div>
        </div>
