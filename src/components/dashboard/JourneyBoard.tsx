    ...
    <div className="flex items-center gap-1 overflow-x-auto py-2 scrollbar-hide">
      {steps.map((step, i) => (
        <div key={step.label} className="flex items-center gap-1 shrink-0">
          ...
          {i < steps.length - 1 && (
            <ArrowLeft className="h-3 w-3 text-muted-foreground shrink-0 rtl:-scale-x-100" />
          )}
        </div>
      ))}
    </div>
...
function AssociationJourney() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const grantBalance = (grants.data ?? []).reduce(
  ...
  const steps: Step[] = [
    ...
    },
    {
      ...
      done: (data?.contracts ?? 0) > 0,
    },
    {
      label: "منح متاحة",
      count: formattedGrants,
      done: (data?.grantBalance ?? 0) > 0,
    },
    {
      label: "مكتملة",
      count: data?.completed ?? 0,
      done: (data?.completed ?? 0) > 0,
    },
  ];
...
function ProviderJourney() {
  ...
  const steps: Step[] = [
    ...
    {
      label: "ساعات عمل",
      count: data?.timeLogs ?? 0,
      done: (data?.timeLogs ?? 0) > 0,
    },
    {
      label: "أرباح",
      count: formattedEarnings,
      done: (data?.earnings ?? 0) > 0,
    },
  ];
...
function DonorJourney() {
  ...
  const steps: Step[] = [
    {
      label: "منح",
      count: data?.donations ?? 0,
      done: (data?.donations ?? 0) > 0,
    },
    {
      label: "إجمالي",
      count: formattedTotal,
      done: (data?.totalAmount ?? 0) > 0,
    },
    {
      label: "تخصيصات",
      count: data?.allocated ?? 0,
      done: (data?.allocated ?? 0) > 0,
    },
    {
      label: "تقارير أثر",
      count: data?.reports ?? 0,
      done: (data?.reports ?? 0) > 0,
    },
  ];
