...
export function DisputeCard({ dispute }: { dispute: any }) {
  ...
  const handleSave = () => {
    updateDispute.mutate(
      { id: dispute.id, status: newStatus, resolution_notes: notes },
      {
        onSuccess: () => {
  ...
  const handleEscrowAction = async (
    action: "release" | "refund" | "freeze" | "unfreeze",
  ) => {
      ...
      <CardHeader className="flex flex-row items-start justify-between gap-2 pb-3">
        <div className="space-y-1">
          <CardTitle className="text-base">
            {dispute.projects?.title ?? "طلب غير معروف"}
          </CardTitle>
          <p className="text-xs text-muted-foreground">
            بواسطة:{" "}
            {dispute.profiles?.organization_name ||
              dispute.profiles?.full_name ||
              "—"}{" "}
            ·{" "}
            {format(new Date(dispute.created_at), "yyyy/MM/dd", { locale: ar })}
          </p>
        </div>
