      ...
      <div className="flex items-center gap-3 rounded-lg border border-border/60 bg-background p-3 text-sm">
        <div className="flex-1 min-w-0 space-y-1">
          {project && (
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to={`/admin/projects/${escrow.project_id}`}
          ...
          {service && (
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to={`/admin/services/${escrow.service_id}`}
                className="text-primary hover:underline font-medium truncate"
              >
                {service.title}
              </Link>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <Badge
              className={`text-[10px] ${statusColors[escrow.status] ?? ""}`}
            >
        ...
        <div className="text-end shrink-0">
          <p className="font-semibold">
            {Number(escrow.amount).toLocaleString()} ر.س
          </p>
        </div>
      </div>
    </div>
