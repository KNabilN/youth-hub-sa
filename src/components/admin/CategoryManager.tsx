...
function exportCSV(items: any[], filename: string) {
  ...
  const rows = items.map(
    (i) =>
      `"${(i.name || "").replace(/"/g, '""')}","${(i.description || "").replace(/"/g, '""')}"`,
  );
...
export function CategoryManager() {
  ...
  const saveEdit = () => {
    if (editId)
      updateMut.mutate({ id: editId, name: editName, description: editDesc });
  };
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {pendingCount > 0 && (
              <Badge variant="destructive">{pendingCount} مقترح</Badge>
            )}
            <Badge variant="secondary">{categories?.length ?? 0}</Badge>
          </div>
        </div>
        <CardDescription>
          تصنيفات الطلبات والخدمات المتاحة على المنصة
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportCSV(categories ?? [], "categories.csv")}
          >
                ...
                onChange={async (e) => {
                  ...
                  const rows = lines
                    ...
                    .map((l) => {
                      const match = l.match(/"([^"]*)","?([^"]*)"?/);
                      return match
                        ? { name: match[1], description: match[2] || null }
                        : null;
                    })
                    .filter(Boolean) as {
                    name: string;
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  هل أنت متأكد من حذف "{c.name}"؟ لا يمكن
