...
const allColumns: ColumnDef[] = [
  ...
  { key: "phone", label: "الهاتف", getValue: (u) => u.phone || "" },
  {
    ...
    label: "المنظمة",
    getValue: (u) => u.organization_name || "",
  },
  {
    key: "role",
    label: "الدور",
    getValue: (u, rm) => roleLabelsMap[rm.get(u.id) ?? ""] || "",
  },
  {
    key: "is_verified",
    label: "التوثيق",
    getValue: (u) => (u.is_verified ? "نعم" : "لا"),
  },
  {
    key: "is_suspended",
    label: "الحالة",
    getValue: (u) => (u.is_suspended ? "معلّق" : "نشط"),
  },
  {
    key: "created_at",
    label: "تاريخ الانضمام",
    getValue: (u) => u.created_at?.slice(0, 10) || "",
  },
  {
    key: "license_number",
    label: "رقم الترخيص",
    getValue: (u) => u.license_number || "",
  },
  { key: "bio", label: "النبذة", getValue: (u) => u.bio || "" },
  {
    key: "contact_officer_name",
    label: "ضابط الاتصال",
    getValue: (u) => u.contact_officer_name || "",
  },
  {
    key: "contact_officer_phone",
    label: "رقم ضابط الاتصال",
    getValue: (u) => u.contact_officer_phone || "",
  },
  {
    key: "contact_officer_email",
    label: "بريد ضابط الاتصال",
    getValue: (u) => u.contact_officer_email || "",
  },
];
...
export function ExportUsersDialog({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const activeCols = allColumns.filter((c) =>
