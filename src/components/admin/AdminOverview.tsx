...
function KPICard({
  ...
  const content = (
    <Card
      className={cn(
        "border-s-4 animate-fade-in transition-shadow",
        c.border,
...
/* ─── Custom Tooltip ─── */
const CustomChartTooltip = React.forwardRef<HTMLDivElement, any>(
...
const chartCardCls =
  "rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-300 border border-border/50 overflow-hidden animate-fade-in";
const gridProps = {
  strokeDasharray: "3 3",
  ...
  const kpis = [
    {
      ...
      color: "primary",
      to: "/admin/users",
      subtitle: stats?.newUsersThisWeek
        ? `+${stats.newUsersThisWeek} هذا الأسبوع`
        : undefined,
    },
    {
      title: "طلبات الجمعيات",
    ...
    {
      title: "ضمان محتجز",
      value: `${(stats?.heldEscrow ?? 0).toLocaleString()} ر.س`,
      icon: Shield,
      color: "primary",
      subtitle: "مبلغ الضمان المالي المحتجز",
      to: "/admin/finance",
    },
    {
        ...
        <Card className={chartCardCls}>
          ...
          <CardContent className="space-y-5 p-6">
            {healthLoading ? (
              ...
              <>
                <HealthMetric
                  label="معدل إكمال الطلبات"
                  value={health?.completionRate ?? 0}
                  color="success"
                />
                <HealthMetric
                  label="معدل حل الشكاوى"
                  value={health?.disputeResolutionRate ?? 0}
                  color="primary"
                />
                <HealthMetric
                  label="نجاح المعاملات المالية"
                  value={health?.escrowSuccessRate ?? 0}
                  color="info"
                />
