...
function CityManager({
  ...
  const addMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cities", regionId] });
      qc.invalidateQueries({ queryKey: ["admin-regions-with-cities"] });
      setName("");
...
export function RegionManager() {
  ...
  const saveEdit = () => {
    if (editId) updateMut.mutate({ id: editId, name: editName });
  };
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportCSV(regions ?? [], "regions.csv")}
          >
            ...
            {(regions ?? []).map((r: any) => {
              ...
              return (
                <Collapsible
                  key={r.id}
                  open={isOpen}
                  onOpenChange={() => toggleRegion(r.id)}
                >
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                هل أنت متأكد من حذف "{r.name}"؟ سيتم حذف جميع
                                المدن التابعة لها.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
