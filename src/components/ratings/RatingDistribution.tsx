...
export function RatingDistribution({ ratings }: RatingDistributionProps) {
  ...
  const avgScores = ratings.map((r) =>
    Math.round((r.quality_score + r.timing_score + r.communication_score) / 3),
  );
        ...
        <div className="flex gap-0.5">
          {[1, 2, 3, 4, 5].map((s) => (
            <Star
              ...
              className={cn(
                "h-4 w-4",
                s <= Math.round(Number(overallAvg))
      ...
      <div className="flex-1 space-y-1.5">
        {distribution.map((d) => (
          <div key={d.star} className="flex items-center gap-2 text-sm">
            ...
            <span className="w-6 text-xs text-muted-foreground text-end">
              {d.count}
            </span>
          </div>
        ))}
