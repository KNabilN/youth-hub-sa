...
export function ProjectForm({
  ...
  const removeSkill = (skill: string) => {
    form.setValue(
      "required_skills",
      values.required_skills.filter((s) => s !== skill),
    );
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <div className="mt-2 mb-3">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {suggestedSkills.map((skill) => {
                        const isSelected = values.required_skills.includes(
                          skill.name,
                        );
                        return (
                          <Badge
                ...
                {values.required_skills.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {values.required_skills
                      .filter(
                        (skill) =>
                          !suggestedSkills?.some((s) => s.name === skill),
                      )
                      .map((skill) => (
                        <Badge
                          key={skill}
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-lg border p-3">
                    <FormLabel className="text-sm">
                      مشروع خاص (لا يظهر لمقدمي الخدمات)
                    </FormLabel>
                    <FormControl>
                      <Switch
                        checked={field.value}
              ...
              {draftProjectId ? (
                <>
                  <p className="text-sm text-muted-foreground">
                    أرفق الملفات المتعلقة بالمشروع (اختياري)
                  </p>
                  <FileUploader
                    entityType="project"
        ...
        {step === 3 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">مراجعة المشروع</CardTitle>
            </CardHeader>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {values.budget && (
                  <>
                    ...
                    <span>{values.budget} ر.س</span>
                  </>
                )}
                {values.estimated_hours && (
                  <>
                    <span className="text-muted-foreground">الساعات:</span>
                    <span>{values.estimated_hours} ساعة</span>
                  </>
                )}
                <span className="text-muted-foreground">خاص:</span>
                <span>{values.is_private ? "نعم" : "لا"}</span>
              </div>
              {values.required_skills.length > 0 && (
                <div>
