...
export function ProjectCard({
  ...
  const pendingBidsCount = (project.bids ?? []).filter(
    (b) => b.status === "pending",
  ).length;
    ...
    <Card className={`card-hover border-t-4 ${borderColor} overflow-hidden`}>
      {(project.categories as any)?.image_url && (
        <div className="w-full h-36 overflow-hidden">
          <img
            src={(project.categories as any).image_url}
            alt={project.title}
        ...
        <div className="space-y-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {pendingBidsCount > 0 && (
              <Badge
                variant="destructive"
                className="h-5 min-w-5 px-1.5 text-[10px] font-bold rounded-full flex items-center gap-0.5 shrink-0"
              >
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {(project as any).cities?.name && (
              <span>• {(project as any).cities.name}</span>
            )}
          </div>
        </div>
      ...
      <CardContent className="space-y-3">
        ...
        {pendingBidsCount > 0 && (
          <div className="text-xs text-primary font-medium bg-primary/5 rounded-md px-2.5 py-1.5 flex items-center gap-1.5">
            ...
            {pendingBidsCount}{" "}
            {pendingBidsCount === 1 ? "عرض جديد" : "عروض جديدة"} بانتظار
            المراجعة
          </div>
        )}
        <div className="flex gap-2 flex-wrap">
          ...
          {(project.status === "suspended" || project.status === "cancelled") &&
            onReactivate && (
              <Button
                size="sm"
                variant="outline"
