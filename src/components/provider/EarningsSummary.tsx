...
> = {
  ...
  frozen: {
    label: "مجمّد",
    variant: "outline",
    border: "border-e-4 border-info/30",
  },
  refunded: {
    label: "مسترد",
    variant: "destructive",
        ...
        <Card className="border-e-4 border-warning/30 bg-gradient-to-l from-amber-500/5 to-amber-500/10">
          ...
          <CardContent>
            <div className="text-xl sm:text-3xl font-bold text-warning">
              {availableBalance.toLocaleString()} ر.س
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="space-y-3">
              {transactions.map((tx) => {
                ...
                const associationName =
                  tx.payer?.organization_name || tx.payer?.full_name;
                ...
                const canWithdraw =
                  tx.status === "released" && !withdrawnEscrowIds.has(tx.id);

                return (
                  <div
                    key={tx.id}
                    className={`p-4 rounded-lg border bg-card hover:bg-muted/30 transition-colors ${st.border}`}
                  >
