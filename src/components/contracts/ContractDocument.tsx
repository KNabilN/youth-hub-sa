...
const LEGAL_CLAUSES = [
  {
    icon: Scale,
    text: "تدار هذه الاتفاقية وتُفسر وفقاً لأنظمة المملكة العربية السعودية.",
  },
  {
    icon: Shield,
    text: "تتولى المنصة الفصل في أي نزاع ينشأ بين الأطراف وفق الآليات المعتمدة.",
  },
  {
    icon: Lock,
    text: "يلتزم الطرفان بالحفاظ على سرية جميع المعلومات المتبادلة خلال فترة التعاقد وبعدها.",
  },
  {
    icon: BookOpen,
    text: "تنتقل حقوق الملكية الفكرية للأعمال المنجزة إلى الطرف الأول (الجمعية) بعد إتمام الدفع.",
  },
  {
    icon: XCircle,
    text: "يحق لأي طرف إلغاء العقد قبل بدء التنفيذ مع استرداد كامل المبلغ عبر نظام الضمان المالي.",
  },
];
...
export function ContractDocument({ data, scopeOverride }: Props) {
  ...
  const amount = Number(
    escrow?.amount ?? acceptedBid?.price ?? project?.budget ?? 0,
  );
  ...
  return (
    <div
      className="bg-card border rounded-xl shadow-sm relative overflow-hidden print:shadow-none print:border-0"
      id="contract-document"
    >
      {/* Watermark */}
      {!isBothSigned && (
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-[0.04] select-none">
          <span className="text-[160px] font-bold rotate-[-25deg] whitespace-nowrap">
            مسودة
          </span>
        </div>
      )}
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              مرجع العقد: <span className="font-mono">{contractRef}</span>
            </p>
            {project?.request_number && (
              <p className="text-sm text-muted-foreground">
                رقم الطلب:{" "}
                <span className="font-mono">{project.request_number}</span>
              </p>
            )}
          </div>
          <div className="text-end space-y-1">
            <Badge
              variant={isBothSigned ? "default" : "outline"}
              className="text-xs"
            >
              {isBothSigned
                ? "موقّع بالكامل"
            ...
            <p className="text-xs text-muted-foreground">
              تاريخ الإنشاء: {fmtDate(contract.created_at)}
            </p>
            {isBothSigned && (
      ...
      <div className="p-6 sm:p-8 space-y-6 relative">
        ...
        <section className="text-sm leading-relaxed">
          <p>
            أُبرم هذا العقد بتاريخ{" "}
            <span className="font-semibold">
              {fmtDate(contract.created_at)}
            </span>{" "}
            بين كل من الطرفين الموضحين أدناه، بناءً على ما تم الاتفاق عليه عبر
            منصة الخدمات المشتركة، ويعد توقيع الطرفين إلكترونياً ملزِماً
            قانونياً ومعتمداً.
          </p>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-lg border bg-muted/30 space-y-1.5">
            <p className="text-xs font-bold text-primary">
              الطرف الأول — الجمعية / المستفيد
            </p>
            <p className="font-semibold">{partyDisplay(association)}</p>
            {association?.user_number && (
              <p className="text-xs text-muted-foreground">
                رقم المستخدم: {association.user_number}
              </p>
            )}
            {association?.license_number && (
              <p className="text-xs text-muted-foreground">
                رقم الترخيص: {association.license_number}
              </p>
            )}
            {association?.contact_officer_name && (
              <p className="text-xs text-muted-foreground">
                مسؤول التواصل: {association.contact_officer_name}
              </p>
            )}
              ...
              {contract.association_signed_at ? (
                <>
                  ...
                  <span className="text-success">
                    وقّع في {fmtDate(contract.association_signed_at)}
                  </span>
                </>
              ) : (
                <>
                  <Clock className="h-3.5 w-3.5 text-muted-foreground" />
                  <span className="text-muted-foreground">لم يوقّع بعد</span>
                </>
              )}
            </div>
          </div>
          <div className="p-4 rounded-lg border bg-muted/30 space-y-1.5">
            ...
            {provider?.user_number && (
              <p className="text-xs text-muted-foreground">
                رقم المستخدم: {provider.user_number}
              </p>
            )}
            {provider?.contact_officer_email && (
              <p className="text-xs text-muted-foreground">
                البريد: {provider.contact_officer_email}
              </p>
            )}
            <div className="flex items-center gap-1 text-xs pt-1">
              {contract.provider_signed_at ? (
                <>
                  <Check className="h-3.5 w-3.5 text-success" />
                  <span className="text-success">
                    وقّع في {fmtDate(contract.provider_signed_at)}
                  </span>
                </>
              ) : (
                <>
        ...
        <section className="space-y-2">
          ...
          <div className="p-3 rounded-lg bg-background border text-sm space-y-1">
            <p>
              <span className="font-semibold">عنوان الخدمة/المشروع:</span>{" "}
              {subjectTitle}
            </p>
            ...
            {project?.regions?.name && (
              <p>
                ...
                {project.regions.name}
                {project?.cities?.name ? ` - ${project.cities.name}` : ""}
              </p>
            )}
            {service?.service_number && (
              <p>
                <span className="font-semibold">رقم الخدمة:</span>{" "}
        ...
        <section className="space-y-2">
          <h2 className="font-bold text-base text-primary">
            المادة الثانية: نطاق العمل والتزامات الطرف الثاني (مقدم الخدمة)
          </h2>
          <div className="p-3 rounded-lg bg-background border text-sm space-y-2">
            ...
            {acceptedBid?.cover_letter && (
              <div className="pt-2 border-t">
                <p className="font-semibold mb-1">
                  بيان مقدم الخدمة (من العرض المقبول):
                </p>
                <p className="whitespace-pre-wrap leading-relaxed text-foreground/90">
                  {acceptedBid.cover_letter}
                </p>
            ...
            <ul className="list-disc list-inside space-y-1 pt-2 border-t text-foreground/90">
              <li>
                تنفيذ الخدمة وفق المواصفات والنطاق المتفق عليه أعلاه بأعلى جودة
                مهنية.
              </li>
              <li>الالتزام بالمدة الزمنية المحددة في المادة الرابعة.</li>
              <li>
                تسليم المخرجات عبر قسم التسليمات في المنصة وإجراء التعديلات
                المعقولة عند الطلب.
              </li>
              <li>
                الحفاظ على سرية بيانات الطرف الأول وعدم استخدامها خارج نطاق
                التعاقد.
              </li>
            </ul>
          </div>
        ...
        <section className="space-y-2">
          <h2 className="font-bold text-base text-primary">
            المادة الثالثة: التزامات الطرف الأول (الجمعية)
          </h2>
          <div className="p-3 rounded-lg bg-background border text-sm">
            <ul className="list-disc list-inside space-y-1 text-foreground/90">
              <li>
                تزويد مقدم الخدمة بالمواد والمعلومات اللازمة لتنفيذ نطاق العمل
                في الوقت المناسب.
              </li>
              <li>سداد قيمة العقد عبر نظام الضمان المالي المعتمد في المنصة.</li>
              <li>
                مراجعة التسليمات خلال مدة معقولة وإبداء الملاحظات بشكل واضح
                ومحدد.
              </li>
        ...
        <section className="space-y-2">
          <h2 className="font-bold text-base text-primary">
            المادة الرابعة: المقابل المالي ومدة التنفيذ
          </h2>
          <div className="p-3 rounded-lg bg-background border text-sm grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {timeline !== null && (
              <div className="flex justify-between gap-2">
                ...
                <span className="font-semibold">{timeline} يوم</span>
              </div>
            )}
            <div className="flex justify-between gap-2">
            ...
            {escrow?.escrow_number && (
              <div className="flex justify-between gap-2">
                <span className="text-muted-foreground">رقم الضمان:</span>
        ...
        <section className="space-y-2">
          <h2 className="font-bold text-base text-primary">
            المادة الخامسة: آلية الضمان والتسليم
          </h2>
          <p className="p-3 rounded-lg bg-background border text-sm leading-relaxed">
            يُحتَجز المبلغ في نظام الضمان المالي للمنصة فور توقيع العقد، ولا
            يُفرَج عنه لمقدم الخدمة إلا بعد قبول الطرف الأول للتسليم النهائي. في
            حال نشوء أي خلاف يحق للطرفين فتح نزاع رسمي عبر المنصة، ويُجمَّد
            الضمان حتى الفصل فيه.
          </p>
        </section>
        ...
        <section className="space-y-2">
          <h2 className="font-bold text-base text-primary">
            المادة السادسة: البنود والأحكام العامة
          </h2>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-lg border-2 border-dashed text-center space-y-1">
            ...
            {contract.association_signed_at ? (
              <>
                <p className="text-success font-semibold text-sm flex items-center gap-1 justify-center">
                  <Check className="h-4 w-4" /> موقّع إلكترونياً
                </p>
                <p className="text-xs text-muted-foreground">
                  {fmtDate(contract.association_signed_at)}
                </p>
              </>
            ) : (
          ...
          <div className="p-4 rounded-lg border-2 border-dashed text-center space-y-1">
            ...
            {contract.provider_signed_at ? (
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  {fmtDate(contract.provider_signed_at)}
                </p>
              </>
            ) : (
              <p className="text-muted-foreground text-sm">
