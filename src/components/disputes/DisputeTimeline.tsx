      ...
      <div className="relative pe-4 border-e-2 border-border space-y-3">
        {logs.map((log: any) => (
          <div key={log.id} className="relative">
            <div
              className={`absolute [inset-inline-end:-13px] top-1.5 h-2.5 w-2.5 rounded-full ${disputeTimelineColors[log.new_status] || "bg-muted-foreground"}`}
            />
            <div className="pe-4">
                ...
                {log.old_status && (
                  <>
                    <span className="text-muted-foreground">
                      {disputeStatusLabels[log.old_status] || log.old_status}
                    </span>
                    <ArrowRight className="h-3 w-3 text-muted-foreground" />
                  </>
                )}
                <span className="font-medium">
                  {disputeStatusLabels[log.new_status] || log.new_status}
                </span>
              </div>
