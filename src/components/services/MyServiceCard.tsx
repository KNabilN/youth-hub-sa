...
const approvalLabels: Record<string, { label: string; variant: "default" | "secondary" | "destructive"; border: string }> = {
 ...
 pending: { label: "قيد المراجعة", variant: "secondary", border: "border-t-4 border-warning/30" },
 approved: { label: "معتمدة", variant: "default", border: "border-t-4 border-success/30" },
 rejected: { label: "مرفوضة", variant: "destructive", border: "border-t-4 border-destructive/30" },
...
export function MyServiceCard({ service, onEdit, onDelete, onSuspend, onReactivate }: MyServiceCardProps) {
 ...
 <Button variant="ghost" size="icon" className="hover:bg-success/10" title="إعادة نشر" onClick={() => onReactivate(service.id)}><Play className="h-4 w-4 text-success" /></Button>
 )}
