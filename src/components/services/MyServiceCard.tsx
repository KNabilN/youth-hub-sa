...
> = {
  ...
  pending: {
    label: "قيد المراجعة",
    variant: "secondary",
    border: "border-t-4 border-warning/30",
  },
  approved: {
    label: "معتمدة",
    variant: "default",
    border: "border-t-4 border-success/30",
  },
  rejected: {
    label: "مرفوضة",
    variant: "destructive",
    border: "border-t-4 border-destructive/30",
  },
  suspended: {
    label: "موقوفة مؤقتاً",
    variant: "destructive",
        ...
        <div className="flex-1 min-w-0">
          ...
          {(service as any).service_number && (
            <Link
              ...
              className="text-sm font-semibold font-mono hover:underline hover:text-primary transition-colors"
            >
              {(service as any).service_number}
            </Link>
          )}
          ...
          {(service.approval === "approved" ||
            ...
            onSuspend && (
              <Button
                ...
                title="إيقاف مؤقت"
                onClick={() => onSuspend(service.id)}
              >
                <Pause className="h-4 w-4 text-orange-500" />
              </Button>
            )}
          {(service.approval === "suspended" ||
            service.approval === "draft" ||
