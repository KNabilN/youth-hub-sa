...
export function BidPaymentDialog({
  open,
  onOpenChange,
  bid,
  projectId,
  projectTitle,
  skipAcceptBid = false,
}: BidPaymentDialogProps) {
  const [step, setStep] = useState(0);
  const [paymentMethod, setPaymentMethod] =
    useState<PaymentMethod>("electronic");
  const [mixedRemainingMethod, setMixedRemainingMethod] = useState<
  ...
  const pricing = useMemo(
    () => calculatePricing(bid.price, commissionRate),
    [bid.price, commissionRate],
  );

  const isAssociation = role === "youth_association";
  const totalGrantAvailable = grantBalance?.total ?? 0;
  const hasFullGrantBalance =
    isAssociation && totalGrantAvailable >= pricing.total;
  const hasPartialGrantBalance =
    isAssociation &&
    totalGrantAvailable > 0 &&
    totalGrantAvailable < pricing.total;
  ...
  const grantCoveragePercent =
    pricing.total > 0
      ? Math.min(100, Math.round((totalGrantAvailable / pricing.total) * 100))
      : 0;
  ...
  const steps = [{ label: "مراجعة التكلفة" }, { label: "الدفع" }];
  ...
  const handleMixedPayment = async () => {
    ...
    try {
      ...
      const grantPortionBase =
        pricing.total > 0
          ? Math.round(
              bid.price * (grantPortionForMixed / pricing.total) * 100,
            ) / 100
          : 0;
      await payFromGrants.mutateAsync({
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <span className="font-medium">
            {(grantBalance?.projectSpecific ?? 0).toLocaleString()} ر.س
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">رصيد المنح العام</span>
          <span className="font-medium">
            {(grantBalance?.general ?? 0).toLocaleString()} ر.س
          </span>
        </div>
        <Separator />
        <div className="flex justify-between font-semibold">
          ...
          <span className="text-primary">
            {totalGrantAvailable.toLocaleString()} ر.س
          </span>
        </div>
      </div>
      ...
      {hasPartialGrantBalance && (
        <div className="flex justify-between text-sm pt-1">
          ...
          <span className="font-bold text-destructive">
            {remainingAfterGrants.toLocaleString()} ر.س
          </span>
        </div>
      )}
    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 rounded-lg bg-muted/50 space-y-1 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {(bid.profiles as any)?.organization_name ||
            ...
            >
              ...
              {hasFullGrantBalance && (
                <div
                  className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${paymentMethod === "grant_balance" ? "border-primary bg-primary/5" : "border-border"}`}
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        الرصيد المتاح: {totalGrantAvailable.toLocaleString()}{" "}
                        ر.س — يغطي كامل التكلفة
                      </p>
                    </div>
                  </Label>
                </div>
              ...
              {hasPartialGrantBalance && (
                <div
                  className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${paymentMethod === "mixed_grant" ? "border-primary bg-primary/5" : "border-border"}`}
                >
                  ...
                  >
                    ...
                    <div>
                      <p className="font-medium text-sm">
                        دفع مختلط (منح + دفع إضافي)
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {grantPortionForMixed.toLocaleString()} ر.س من المنح +{" "}
                        {remainingAfterGrants.toLocaleString()} ر.س متبقي
                      </p>
                    </div>
                  </Label>
                </div>
            ...
            {paymentMethod === "electronic" && (
              ...
              >
                ...
                {loadingPayment || acceptBid.isPending
                  ? "جاري المعالجة..."
                  : skipAcceptBid
                    ? "المتابعة للدفع"
                    : "قبول العرض والمتابعة للدفع"}
              </Button>
            )}
            ...
            {paymentMethod === "bank_transfer" && (
              <div className="space-y-3">
                ...
                >
                  {bankTransfer.isPending || acceptBid.isPending
                    ? "جاري المعالجة..."
                    : skipAcceptBid
                      ? "إرسال الإيصال"
                      : "قبول العرض وإرسال الإيصال"}
                </Button>
              </div>
            )}
            ...
            {paymentMethod === "grant_balance" && (
              <Button
                ...
                disabled={
                  loadingPayment ||
                  acceptBid.isPending ||
                  payFromGrants.isPending
                }
                onClick={handleGrantPayment}
              >
                <Wallet className="h-4 w-4 me-1" />
                {loadingPayment || payFromGrants.isPending
                  ? "جاري المعالجة..."
                  : skipAcceptBid
                    ? "الدفع من المنح"
                    : "قبول العرض والدفع من المنح"}
              </Button>
            )}
              ...
              <div className="space-y-4">
                <div className="rounded-lg border bg-muted/30 p-3 space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-success">
                      {grantPortionForMixed.toLocaleString()} ر.س
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">المتبقي للدفع</span>
                ...
                <div className="space-y-2">
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(v) =>
                      setMixedRemainingMethod(
                        v as "electronic" | "bank_transfer",
                      )
                    }
                    className="space-y-2"
                  >
                    <div
                      className={`flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer transition-colors ${mixedRemainingMethod === "electronic" ? "border-primary bg-primary/5" : "border-border"}`}
                    >
                      <RadioGroupItem
                        value="electronic"
                        id="mixed-electronic"
                    ...
                    <div
                      className={`flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer transition-colors ${mixedRemainingMethod === "bank_transfer" ? "border-primary bg-primary/5" : "border-border"}`}
                    >
                ...
                <Button
                  ...
                  disabled={
                    ...
                    acceptBid.isPending ||
                    payFromGrants.isPending ||
                    (mixedRemainingMethod === "bank_transfer" && !receiptFile)
                  }
