...
function isWithDiscount(
  p: PricingBreakdown | PricingWithDiscount,
): p is PricingWithDiscount {
  return "discount" in p && (p as PricingWithDiscount).discount > 0;
}

export function PricingBreakdownDisplay({
  pricing,
  className,
    ...
    <div className={className}>
        ...
        {hasDiscount && (
          <div className="flex justify-between text-primary">
            ...
            <span className="font-bold">
              −{pricing.discount.toLocaleString()} ر.س
            </span>
          </div>
        )}
      </div>
      ...
      {hasDiscount && (
        <p className="text-xs text-muted-foreground mt-1">
          مستحقات المزود: {pricing.originalSubtotal.toLocaleString()} ر.س (كامل
          المبلغ الأصلي)
        </p>
      )}
    </div>
