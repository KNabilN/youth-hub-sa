...
export function useAdminStats() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const heldEscrow = (escrowRes.data ?? []).reduce(
...
export function useAdminGrowthData() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [usersRes, projectsRes, escrowRes, donationsRes] =
        await Promise.all([
          supabase
            .from("profiles")
            .select("created_at")
      ...
      const monthlyData: Record<
        string,
        { users: number; projects: number; escrow: number; donations: number }
      > = {};
