  ...
  return useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("projects")
        .select(
          "*, categories(*), regions(*), cities(*), bids!bids_project_id_fkey(id, status)",
        )
...
export function useCreateProject() {
  ...
  return useMutation({
    mutationFn: async (
      values: Omit<TablesInsert<"projects">, "association_id">,
    ) => {
      ...
      if (error)
        throw new Error(
          getFriendlyDatabaseError(error, "حدث خطأ أثناء إنشاء الطلب"),
        );
      return data;
    },
...
export function useUpdateProject() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (error)
        throw new Error(
          getFriendlyDatabaseError(error, "حدث خطأ أثناء تحديث الطلب"),
        );
      return data;
    },
    onSuccess: (data) => {
...
export function useProjectStats() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const pendingHours =
        (timeLogsRes.data as { hours: number }[] | null)?.reduce(
      ...
      const avgRating = ratings.length
        ? (
            ratings.reduce(
              (s, r) =>
                s +
                (r.quality_score + r.timing_score + r.communication_score) / 3,
              0,
            ) / ratings.length
          ).toFixed(1)
        : "0";
