...
export function usePayFromGrants() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      try {
        return await executeGrantPayment(
          user.id,
          amount,
          totalAmount,
          payeeId,
...
async function executeGrantPayment(
  userId: string,
  amount: number,
  totalAmount: number,
  payeeId: string,
  ...
  if (projectId) {
    const { data, error } = await supabase
      ...
      .select(
        "id, amount, donation_status, donor_id, association_id, project_id, service_id",
      )
    ...
    for (const c of contributions) {
      ...
      if (cAmount <= remaining) {
        ...
        const { error } = await supabase
          ...
          .eq("id", c.id);
        if (error) {
          console.error("consume full contribution error:", error);
          throw error;
        }
        ...
        const { error: upErr } = await supabase
          ...
          .eq("id", c.id);
        if (upErr) {
          console.error("update leftover error:", upErr);
          throw upErr;
        }
        ...
        if (insErr) {
          console.error("insert consumed portion error:", insErr);
          throw insErr;
        }
      }
    }
